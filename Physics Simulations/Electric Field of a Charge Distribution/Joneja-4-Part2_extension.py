...
efma = empty([points,points],float)  #electric field magnitude analytic
efda = empty([points,points],float)  #electric field direction analytic
...
#returns mag and direction of E field at point(x,y) 
#using analytically derived partial derivatives
def efield_analytic(x,y):
    if (x == x1 and y == y1) or (x == x2 and y == y2):
        e_x,e_y = 0.0,0.0
    else:
        ...
        e_x = k*(q1*(x-x1)*r1 + q2*(x-x2)*r2)
        e_y = k*(q1*(y-y1)*r1 + q2*(y-y2)*r2) 

#        Equivalent to:
#        e1_x = (-1/(4*pi*epsilon0))*q1*(x-x1)*((x-x1)**2+(y-y1)**2)**-1.5
#        e2_x = (-1/(4*pi*epsilon0))*q2*(x-x2)*((x-x2)**2+(y-y2)**2)**-1.5
#        e1_y = (-1/(4*pi*epsilon0))*q1*(y-y1)*((x-x1)**2+(y-y1)**2)**-1.5
#        e2_y = (-1/(4*pi*epsilon0))*q2*(y-y2)*((x-x2)**2+(y-y2)**2)**-1.5
#        e_x = e1_x + e2_x
#        e_y = e1_y + e2_y
...
for i in range(points):
    ...
    for j in range(points):
        ...
        efma[j,i],efda[j,i] = efield_analytic(x,y)
...
figure()
imshow(efma,origin="lower",extent=[0,side,0,side])
gray()
title("Electric Field Magnitude -- Computed Analytically")
show()

figure()
imshow(efda,origin="lower",extent=[0,side,0,side])
hsv()
title("Electric Field Direction -- Computed Analytically")
colorbar()
show()
